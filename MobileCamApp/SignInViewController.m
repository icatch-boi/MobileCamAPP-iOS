...
- (void)viewDidLoad {
  ...
  self.credentialsButton.accessibilityIdentifier = kCredentialsButtonAccessibilityIdentifier;
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(returnBackToPVClick)];
}

- (void)returnBackToPVClick {
...
- (void)signIn:(GIDSignIn *)signIn
  ...
  if (error) {
    ...
    AppLogDebug(AppLogTagAPP, @"%@",_signInAuthStatus.text );
    return;
  }
  [self reportAuthStatus];
  [self updateButtons];
